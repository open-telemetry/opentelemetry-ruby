...
| `encoding:`               | `OTEL_EXPORTER_OTLP_PROTOCOL`                | `"protobuf"`                        |
...
`encoding:` selects the OTLP encoding: `"protobuf"` (the default) sends binary Protobuf, and `"json"` sends spec-compliant OTLP/JSON. When not passed explicitly, it is derived from `OTEL_EXPORTER_OTLP_PROTOCOL` (`http/protobuf` or `http/json`).
