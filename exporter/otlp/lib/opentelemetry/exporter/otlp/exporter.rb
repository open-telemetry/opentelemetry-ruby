    ...
    module OTLP
      ...
      class Exporter # rubocop:disable Metrics/ClassLength
        ...
        def send_bytes(bytes, timeout:) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/MethodLength, Metrics/PerceivedComplexity
          ...
          @metrics_reporter.record_value(Attributes::OTEL_OTLP_EXPORTER_MESSAGE_UNCOMPRESSED_SIZE, value: bytes.bytesize)

          request = Net::HTTP::Post.new(@path)
          if @compression == 'gzip'
            request.add_field('Content-Encoding', 'gzip')
            body = Zlib.gzip(bytes)
            @metrics_reporter.record_value(Attributes::OTEL_OTLP_EXPORTER_MESSAGE_COMPRESSED_SIZE, value: body.bytesize)
          else
            body = bytes
          end
          request.body = body
          ...
          around_request do
            ...
            when Net::HTTPNotFound
              ...
              @metrics_reporter.add_to_counter(Attributes::OTEL_OTLP_EXPORTER_FAILURE, labels: { 'reason' => response.code })
              FAILURE
            ...
            @metrics_reporter.add_to_counter(Attributes::OTEL_OTLP_EXPORTER_FAILURE, labels: { 'reason' => e.class.to_s })
            return FAILURE
          end
        ensure
        ...
        def measure_request_duration
          ...
          begin
            ...
            @metrics_reporter.record_value(Attributes::OTEL_OTLP_EXPORTER_REQUEST_DURATION,
                                           value: duration_ms,
        ...
        def backoff?(retry_count:, reason:, retry_after: nil) # rubocop:disable Metrics/CyclomaticComplexity, Metrics/MethodLength, Metrics/PerceivedComplexity
          @metrics_reporter.add_to_counter(Attributes::OTEL_OTLP_EXPORTER_FAILURE, labels: { 'reason' => reason })
