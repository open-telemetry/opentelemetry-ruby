...
describe OpenTelemetry::Exporter::OTLP::Exporter do
  ...
  describe '#export' do
    ...
    it 'translates all the things' do
      ...
      span = with_ids(trace_id, child_span_id) { tracer.start_span('child', with_parent_context: root_ctx, kind: :producer, start_timestamp: start_timestamp + 1, links: [OpenTelemetry::Trace::Link.new(root.context, 'attr' => 4)]) }
      span['b'] = true
      span['f'] = 1.1
      span['i'] = 2
      span['s'] = 'val'
      ...
      client = with_ids(trace_id, client_span_id) { tracer.start_span('client', with_parent_context: child_ctx, kind: :client, start_timestamp: start_timestamp + 2).finish(end_timestamp: end_timestamp) }
      client_ctx = tracer.with_span(client) { |_, ctx| ctx }
      with_ids(trace_id, server_span_id) { other_tracer.start_span('server', with_parent_context: client_ctx, kind: :server, start_timestamp: start_timestamp + 3).finish(end_timestamp: end_timestamp) }
      span.add_event('event', attributes: { 'attr' => 42 }, timestamp: start_timestamp + 4)
      with_ids(trace_id, consumer_span_id) { tracer.start_span('consumer', with_parent_context: child_ctx, kind: :consumer, start_timestamp: start_timestamp + 5).finish(end_timestamp: end_timestamp) }
