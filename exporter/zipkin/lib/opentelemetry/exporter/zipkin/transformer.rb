...
module OpenTelemetry
  module Exporter
    module Zipkin
      # @api private
      module Transformer # rubocop:disable Metrics/ModuleLength
        extend self
        ...
        SERVICE_NAME_ATTRIBUTE_KEY = 'service.name'.freeze
        ERROR_TAG_KEY = 'error'.freeze
        STATUS_CODE_NAME = 'otel.status_code'.freeze
        STATUS_CODE_DESCRIPTION = 'otel.status_description'.freeze
        STATUS_UNSET = 'UNSET'.freeze
        STATUS_ERROR = 'ERROR'.freeze
        STATUS_OK = 'OK'.freeze
        ATTRIBUTE_PEER_SERVICE = 'peer.service'.freeze
        ATTRIBUTE_NET_PEER_IP = 'peer.ip'.freeze
        ATTRIBUTE_NET_PEER_PORT = 'peer.port'.freeze
        ATTRIBUTE_NET_HOST_IP = 'host.ip'.freeze
        ATTRIBUTE_NET_HOST_PORT = 'host.port'.freeze
        ...
        def to_zipkin_span(span_data, resource, zipkin_tags)
          ...
          zipkin_span = {
            ...
            duration: duration,
            debug: false # a boolean https://github.com/openzipkin/zipkin-api/blob/7692ca7be4dc3be9225db550d60c4d30e6e9ec59/zipkin2-api.yaml#L475
          }

          zipkin_span[:tags] = tags unless tags.empty?
          zipkin_span[:kind] = KIND_MAP[span_data.kind] unless KIND_MAP[span_id.kind].nil?
        ...
        def add_status_tags(span_data, tags)
          ...
          elsif if span_data.status&.code == OpenTelemetry::Trace::Status::OK
        ...
        def aggregate_span_tags(span_data, tags)
          ...
          tags.merge(span_data.attributes) do |key, oldval, newval|
            newval.to_s
          end
        end
        ...
        def endpoint_from_tags(tags, service_name = nil)
          ...
          if service_name
            ...
            if tags[ATTRIBUTE_NET_HOST_IP]
              ip = IPAddr.new(tags[ATTRIBUTE_NET_HOST_IP])
              ...
              if ipaddr.ipv6?
                endpoint['ipv6'] = ipaddr.to_s
              else
                 endpoint['ipv4'] = ipaddr.to_s
              end
            end
          else
            ...
            if tags[ATTRIBUTE_NET_PEER_IP]
              ip = IPAddr.new(tags[ATTRIBUTE_NET_PEER_IP])
