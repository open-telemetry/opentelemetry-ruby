...
describe OpenTelemetry::Exporter::OTLP::Logs::LogsExporter do
  ...
  describe '#initialize' do
    ...
    it 'only allows protobuf or json encoding' do
      ['grpc', nil].each do |encoding|
        assert_raises ArgumentError do
          OpenTelemetry::Exporter::OTLP::Logs::LogsExporter.new(encoding: encoding)
        end
      end
      ...
      %w[protobuf json].each do |encoding|
        exp = OpenTelemetry::Exporter::OTLP::Logs::LogsExporter.new(encoding: encoding)
  ...
  describe '#export' do
    ...
    it 'encodes as JSON when encoding is json' do
      exp = OpenTelemetry::Exporter::OTLP::Logs::LogsExporter.new(encoding: 'json', compression: 'none')
