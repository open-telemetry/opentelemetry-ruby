  ...
  module Config
    class << self
      ...
      def build_resource(resource_cfg)
        ...
        OpenTelemetry.logger.warn('Config: schema_url is supported; ignoring.') if resource_cfg.schema_url
      ...
      def run_detector(name)
        ...
        when 'google_cloud_platform'
          ...
          OpenTelemetry.logger.warn("Config: unknown resource detector '#{name}'; skipping.")
      ...
      def detect_resource(class_name, *)
        ...
        OpenTelemetry.logger.warn("Config: resource detector '#{class_name}' is not available — is the gem installed?")
        OpenTelemetry::SDK::Resources::Resource.create({})
      end
    end
  end
