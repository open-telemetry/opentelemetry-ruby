...
module SchemaDownloader
  ...
  def self.fetch_schema(url, dest_dir)
    ...
    raw = URI.open(url, 'rb', 'User-Agent' => 'opentelemetry-config-generator').read # rubocop:disable Security/Open
...
module ConstantsGenerator
  ...
  # Nests the generated struct definitions under OpenTelemetry::Config::Model
