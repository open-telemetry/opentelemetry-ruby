...
OpenTelemetry::SDK.configure do |c|
  c.use 'OpenTelemetry::Instrumentation::Manticore', 'allowed_request_headers' => ['Connection']
...
async_resp1.on_complete do |r|
  puts '*' * 20, "parallel call #{r.request.uri}"
...
request.on_complete do |r|
  puts '*' * 20, "background call #{r.request.uri}"
  puts "response code: #{r.code}"
end
future = request.call
response = future.get
