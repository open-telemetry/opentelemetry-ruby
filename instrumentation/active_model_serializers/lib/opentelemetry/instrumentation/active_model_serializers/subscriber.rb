...
module OpenTelemetry
  module Instrumentation
    module ActiveModelSerializers
      # Event handler class for ActiveModelSerializers Ruby driver
      class Subscriber
        THREAD_KEY = :__opentelemetry_active_model_serializers_spans__
        ...
        def started(event)
          ...
          # https://github.com/active_model_serializersdb/active_model_serializers-ruby-driver/blob/master/lib/active_model_serializers/monitoring.rb#L70
          # https://github.com/active_model_serializersdb/active_model_serializers-ruby-driver/blob/master/lib/active_model_serializers/monitoring/publishable.rb#L38-L56
          collection = get_collection(event.command)
          attributes = build_attributes(event)
          attributes['db.active_model_serializersdb.collection'] = collection if collection
        ...
        def failed(event)
          finish_event('failed', event) do |span|
            if event.is_a?(::ActiveModelSerializers::Monitoring::Event::CommandFailed)
        ...
        def finish_event(name, event)
          ...
          OpenTelemetry.logger.debug("error when handling ActiveModelSerializersDB '#{name}' event: #{e}")
        ensure
        ...
        def build_attributes(event)
          {
            'db.system' => 'active_model_serializersdb',
