          ...
          module DeliverMessages
            ...
            def subscribe!
              ::ActiveSupport::Notifications.subscribe('deliver_messages.producer.kafka') do |_name, start, finish, _id, payload|
                ...
                attributes['delivered_message_count'] = payload[:delivered_message_count] if payload.key?(:delivered_message_count)
                attributes['attempts'] = payload[:attempts] if payload.key?(:attempts)
