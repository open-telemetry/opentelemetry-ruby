...
* Add Gemfile, opentelemetry-instrumentations-#{name}.gemspec
* Add runnable (docker) example (using its own Gemfile)
```
$ docker-compose run ex-instrumentations-myinstrumentation bundle install
$ docker-compose run ex-instrumentations-myinstrumentation
...
* `instrumentations/faraday` and `instrumentations/sinatra` were the earliest, simplest implementations
...
* Gemfile should include "opentelemetry-instrumentations-#{instrumentation_name}"
