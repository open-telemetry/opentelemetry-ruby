...
describe OpenTelemetry::Logs::LegacyGlobalCompat do
  # Each case runs in a subprocess. The shim requires
  # opentelemetry/logs/global on first use, which cannot be undone, and the
  # legacy SDK it detects is simulated by defining a constant.
  LEGACY_SDK = <<~RUBY
    module OpenTelemetry
      module SDK
        module Logs
          class LoggerProvider; end
        end
      end
    end
  RUBY
  ...
  def run_script(body, legacy_sdk: false)
    script = +"require 'opentelemetry-logs-api'\n"
  ...
  describe 'without a logs SDK' do
    it 'leaves the top-level accessor undefined' do
      ...
      assert_predicate(status, :success?, 'expected NoMethodError from OpenTelemetry.logger_provider')
    end

    it 'leaves the top-level writer undefined' do
      _, status = run_script(<<~RUBY)
        begin
          OpenTelemetry.logger_provider = OpenTelemetry::Logs::LoggerProvider.new
          exit 1
        rescue NoMethodError
          exit 0
        end
  ...
  describe 'with a legacy logs SDK' do
    it 'registers a provider through the top-level writer' do
      _, status = run_script(<<~RUBY, legacy_sdk: true)
        ...
        exit(OpenTelemetry::Internal.logger_provider.equal?(provider) ? 0 : 1)
      RUBY
    ...
    it 'reads the provider through the top-level accessor' do
      _, status = run_script(<<~RUBY, legacy_sdk: true)
        ...
        exit(OpenTelemetry.logger_provider.equal?(provider) ? 0 : 1)
      RUBY
    ...
    it 'reports respond_to? for the accessors' do
      _, status = run_script(<<~RUBY, legacy_sdk: true)
        exit(OpenTelemetry.respond_to?(:logger_provider) && OpenTelemetry.respond_to?(:logger_provider=) ? 0 : 1)
      RUBY
  ...
  describe 'what it adds to the OpenTelemetry singleton' do
    it 'installs the two hooks and nothing else' do
      _(OpenTelemetry::Logs::LegacyGlobalCompat.private_instance_methods.sort)
        .must_equal(%i[method_missing respond_to_missing?])
    end
  end

  # Safe to run in process: the shim declines any name outside ACCESSORS
  # regardless of whether opentelemetry/logs/global has been required.
  ...
  describe 'unrelated methods' do
    it 'raises without a logs SDK' do
      _, status = run_script(<<~RUBY)
        begin
    ...
    it 'raises with a legacy logs SDK' do
      _, status = run_script(<<~RUBY, legacy_sdk: true)
        begin
          OpenTelemetry.definitely_not_a_method
          exit 1
