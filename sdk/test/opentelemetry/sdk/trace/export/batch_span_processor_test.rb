...
describe OpenTelemetry::SDK::Trace::Export::BatchSpanProcessor do
  ...
  describe 'initialization' do
    ...
    it 'spawns a thread on boot if OTEL_START_EXPORT_THREAD_ON_BOOT is true' do
      mock = MiniTest::Mock.new
      mock.expect(:call, nil)

      Thread.stub(:new, mock) do
        with_env('OTEL_START_EXPORT_THREAD_ON_BOOT' => 'true') do
          BatchSpanProcessor.new(exporter: TestExporter.new)
        end
      end
    ...
    it 'does not spawn a thread on boot if OTEL_START_EXPORT_THREAD_ON_BOOT is false' do
      mock = MiniTest::Mock.new
      mock.expect(:call, nil) { assert false }

      Thread.stub(:new, mock) do
        with_env('OTEL_START_EXPORT_THREAD_ON_BOOT' => 'false') do
          BatchSpanProcessor.new(exporter: TestExporter.new)
        end
      end
    end
